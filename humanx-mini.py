...
class MetaCognitiveSystem:
    ...
    def self_train(self, new_data: List[str]) -> None:
        ...
        already existing conversation data. 
        """
...
class CognitiveArchitecture:
    ...
    def process_input(self, input_text: str, current_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'memory_context': [self._serialize_memory(m) for m in self._get_relevant_memories(input_text)]
        }
...
class ChatbotConfig:
    ...
    max_new_tokens: int = 4000      # For GPU
    cpu_max_new_tokens: int = 3500   # For CPU
    temperature: float = 0.2

# ----- NEW MATH HELPER FUNCTION -----
def parse_and_solve_math(user_input: str) -> (bool, str):
    """
    Attempts to parse the user_input as a math expression and compute the result.
    Returns (True, result_string) if successful, or (False, "") if not purely math.
    """
    try:
        expression = sympify(user_input)   # Convert to Sympy expression
        result = expression.evalf()        # Evaluate numerically
...
class Chatbot:
    ...
    def generate_response(self, prompt: str) -> str:
        ...
        with torch.inference_mode():
            output_ids = self.model.generate(
                ...
                max_new_tokens=400 if self.device.type == "cpu" else 200,
    ...
    async def chat(self, user_input: str) -> str:
        """
        The main chat method, now including math parsing, step-by-step explanation,
        and AGI-based reasoning in the final answer.
        """
        # 1) Cognitive architecture as usual
        cognitive = self.cognitive_architecture.process_input(user_input, {})

        # 2) Try parsing user input as math
        is_math, math_answer = parse_and_solve_math(user_input)
        if is_math:
            # Attach the numeric result to the cognitive dictionary
        ...
        # 4) If we recognized math, generate a short step-by-step explanation
        explanation = ""
        if is_math:
            prompt_for_explanation = (
                f"The user asked: {user_input}\n"
                f"I computed the numeric result as {math_answer}.\n"
                "Explain how to arrive at that result step by step."
            )
            explanation = self.generate_response(prompt_for_explanation)

        # 5) Combine everything into the final context for your LLM
        combined_context = (
        ...
        if is_math:
            combined_context += f"Step-by-step explanation: {explanation}\n"

        # 6) Generate the final answer, referencing all context
        prompt = f"User: {user_input}\nContext: {combined_context}\nChatbot:\n"
        response = self.generate_response(prompt)

        # 7) Learn from conversation, handle memory, etc.
